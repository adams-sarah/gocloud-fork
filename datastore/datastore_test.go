...
var testCases = []testCase{
	...
	},
	{
		...
		"datastore: __key__ field found at top level struct",
		"",
	},
	{
...
func sortPL(pl PropertyList) {
	...
	for i := range pl {
		if nent, ok := pl[i].Value.(*Entity); ok {
			sortPL(nent.Properties)
		} else if niface, ok := pl[i].Value.([]interface{}); ok {
			for j := range niface {
				if nent, ok := niface[j].(*Entity); ok {
					sortPL(nent.Properties)
				}
			}
		}
